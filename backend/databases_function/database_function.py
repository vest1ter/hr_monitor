...
def get_resumes(
    ...
    if sort_by == "created_at":
        query = query.order_by(
            asc(Resume.uploaded_at) if order == "asc" else desc(Resume.uploaded_at)
        )
    elif sort_by == "sla":
        query = query.join(Stage).order_by(
            asc(Stage.sla) if order == "asc" else desc(Stage.sla)
        )
